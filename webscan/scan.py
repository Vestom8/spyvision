#!/usr/bin/env python3
"""Точка входа сканера Spyvision (копия для запуска из папки webscan).
...
def _project_root() -> Path:
    ...
    for candidate in (here, *here.parents):
        if (candidate / "webscan" / "__init__.py").is_file():
            return candidate
    # scan.py лежит внутри самого пакета webscan/
    if (here / "__init__.py").is_file() and here.name == "webscan":
        return here.parent
    raise SystemExit(
