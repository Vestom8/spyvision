#!/usr/bin/env python3
"""Точка входа сканера Spyvision.

Запуск из корня проекта или через копии scan.py в подпапках:
    python scan.py
    python scan.py --cli
...
def _project_root() -> Path:
    ...
    raise SystemExit(
        ...
        "Запускайте: python scan.py из папки spiyvision или её подпапок."
    )
...
def _bootstrap() -> None:
    ...
    # Рабочая директория = корень проекта: index.html, report.html, bg.jfif рядом
    try:
        os.chdir(root_s)
